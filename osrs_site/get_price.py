...
def create_api():
	global APIDATA
	APIDATA  = requests.get('http://prices.runescape.wiki/api/v1/osrs/latest', headers={'User-Agent': 'gbc_collib\'s amateur price tracker', 'From': 'collinstasiak@gmail.com'})
...
class ItemData():
	...
	def name_to_id(self):
		with open('data_base.json', 'r') as f:
			data = json.load(f)
		for item in data:
			if ''.join(item['name'].lower().split()) == ''.join(self.item_id.split().lower()): #checks name to item id  database
	...
	def id_to_name(self):
		with open(DATABASE, 'r') as f:
			data = json.load(f)
		for item in data:
			if str(item['id']) == self.item_id: #checks name to item id database and normalizes caps to make user friendly
...
def main(args):
	...
	print(requested_item_data.item_name + 's profit margin is ' + str(requested_item_data.high - requested_item_data.low) + ' at ' + time_retrieved.strftime("%H:%M:%S"))
	return 0
