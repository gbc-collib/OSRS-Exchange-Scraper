...
def grab_alc_price():
	with open(ITEM_DATABASE) as f:  # opens prebuilt item database and closes as soon as it's imported
		...
		for item in items_list:
			if not 'highalch' in item or not 'limit' in item: #handles edge case of non-alch-able items or non tradable
				continue
			high_alc_values[item['name']] = item['highalch'], item['limit']
	return high_alc_values

def alc_profit(items: dict = grab_alc_price(), margin: int = 10, limit: int = 1000):  # takes dict keyed by item name with value of high alc, defaults to all items
...
def sort_alc_list(list: dict):
	return dict(sorted(list.items(), key=lambda item: item[1], reverse=True))
