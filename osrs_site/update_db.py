...
since we are not saving price data we will never have to update this except when new items are added.
"""
...
def populate_database(connection, data):
    ...
    #res = connection.execute("SELECT * FROM items_info")
    #rint(res.fetchone())
...
def main(args):
    ...
    populate_database(connection, data)
    res = connection.execute("SELECT item_name FROM items_info;")
    print(res.fetchall())
    connection.close()
